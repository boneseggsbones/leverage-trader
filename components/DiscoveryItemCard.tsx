...
const DiscoveryItemCard: React.FC<DiscoveryItemCardProps> = ({ item, owner, onClick, isWishlisted, onToggleWishlist }) => {
    ...
    return (
        <div
            onClick={onClick}
            className="w-full text-left bg-white dark:bg-gray-800 rounded-lg overflow-hidden border border-gray-200 dark:border-gray-700 hover:shadow-xl hover:-translate-y-1 transition-all duration-300 cursor-pointer"
        >
            ...
            <div className="p-4">
                ...
                <p className="text-sm text-gray-500 dark:text-gray-400">{formatCurrencyOptional(item.estimatedMarketValue ?? null)}</p>

                <div className="mt-4 pt-4 border-t border-gray-100 dark:border-gray-700">
                    <p className="text-xs text-gray-400">Owner</p>
                    <button onClick={handleOwnerClick} className="w-full text-left">
                        <div className="flex justify-between items-center">
                            <p className="text-sm font-semibold text-gray-700 dark:text-gray-200 hover:underline">{owner.name}</p>
                            <div className="text-sm">
                                <span title="Reputation Score" className="font-semibold text-blue-600 dark:text-blue-400">{owner.valuationReputationScore}</span>
                                <span className="text-blue-400 dark:text-blue-500"> Rep</span>
                            </div>
                        </div>
                    </button>
                </div>
