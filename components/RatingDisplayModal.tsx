            ...
            {rating ? (
                <div className="space-y-4">
                    <div className="flex justify-between items-center">
                     ...
                     <div className="flex justify-between items-center">
                        <span className="font-medium text-gray-700">Item Accuracy:</span>
                     ...
                     <div className="flex justify-between items-center">
                        <span className="font-medium text-gray-700">Shipping Speed:</span>
                        <StarDisplay value={rating.shippingSpeedScore} />
                    </div>
                    {rating.publicComment && (
                        <div className="pt-2">
                             <p className="font-medium text-gray-700 text-sm">Public Comment:</p>
                             <p className="text-gray-600 italic mt-1 text-sm bg-white p-2 rounded-md">"{rating.publicComment}"</p>
                        </div>
                    )}
                </div>
            ) : (
                <p className="text-gray-500 text-center py-8">No rating was submitted.</p>
            )}
        </div>
    );
};
...
const RatingDisplayModal: React.FC<RatingDisplayModalProps> = ({ isOpen, onClose, ratings, trade, currentUser, otherUser }) => {
    ...
    return (
        ...
        >
            <div 
                className="bg-white rounded-lg shadow-xl p-6 w-full max-w-3xl m-4 transform transition-all"
                onClick={e => e.stopPropagation()}
            >
                <h3 className="text-xl font-bold text-slate-800 mb-2">Trade Feedback</h3>
                <p className="text-sm text-slate-500 mb-6">Ratings for your trade with {otherUser.name}.</p>
